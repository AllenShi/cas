...
public class HazelcastHealthIndicator extends AbstractCacheHealthIndicator {
    ...
    protected CacheStatistics[] getStatistics() {
        ...
        val clusterSize = instance.getOriginal().node.getClusterService().getSize();
        val isMaster = instance.getOriginal().node.isMaster();
        ...
        instance.getConfig().getMapConfigs().keySet().forEach(key -> {
            ...
            LOGGER.trace("Starting to collect hazelcast statistics for map [{}] identified by key [{}]...", map, key);
