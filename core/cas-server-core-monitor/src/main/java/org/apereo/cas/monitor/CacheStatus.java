    ...
    private static String buildDescription(final String desc, final CacheStatistics... statistics) {
        ...
        return Stream.of(statistics)
                ...
                .collect(Collectors.joining("|",
                        sb.toString() + "Cache statistics: [\n\t  Members: "+statistics[0].getNumberOfMembers()+
                                "\n\t  Is Master: "+statistics[0].isMaster(),"]"));
    }

    private static long totalMemory(CacheStatistics... statistics) {
        long total = 0;
        for(CacheStatistics stat : statistics) {
            total += stat.getMemoryCost();
        }
        return total;
    }
}
