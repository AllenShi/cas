...
public class MultiTimeUseTGTOrTimeoutExpirationPolicy extends AbstractCasExpirationPolicy {
     ...
     */
    private static final Logger LOGGER = LoggerFactory.getLogger(MultiTimeUseTGTOrTimeoutExpirationPolicy.class);
     ...
     */
    private MultiTimeUseTGTOrTimeoutExpirationPolicy() {
     ...
     */
    @JsonCreator
    public MultiTimeUseTGTOrTimeoutExpirationPolicy(@JsonProperty("numberOfUses") final int numberOfUses,
    ...
    public boolean isExpired(final TicketState ticketState) {
        ...
        if (countUses >= this.numberOfUses) {
            LOGGER.warn("Ticket has been expired for exceeding usage count [{}] for user [{}]", this.numberOfUses, ticketState.getAuthentication().getPrincipal().getId());
            return true;
        }
    ...
    public boolean equals(final Object obj) {
        ...
        final MultiTimeUseTGTOrTimeoutExpirationPolicy rhs = (MultiTimeUseTGTOrTimeoutExpirationPolicy) obj;
     ...
     */
    @JsonTypeInfo(use=JsonTypeInfo.Id.CLASS, include= JsonTypeInfo.As.PROPERTY)
    public static class ProxyTicketExpirationPolicy extends MultiTimeUseTGTOrTimeoutExpirationPolicy {
     ...
     */
    @JsonTypeInfo(use=JsonTypeInfo.Id.CLASS, include= JsonTypeInfo.As.PROPERTY)
    public static class ServiceTicketExpirationPolicy extends MultiTimeUseTGTOrTimeoutExpirationPolicy {
